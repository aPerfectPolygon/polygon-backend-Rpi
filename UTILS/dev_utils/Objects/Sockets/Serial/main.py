...
class Serial(sck_utils.BasicSocket):
	def __init__(self, reader_writer: ty.Tuple[StreamReader, StreamWriter], **kwargs):
		...
		self.serial = self.writer._transport._serial
	...
	def is_closed(self):
		...
		"""
		try:
			return (not self.serial.is_open) or self.serial.out_waiting < 0
		except:
			return True
...
async def connect(port: str, baudrate: int, **kwargs) -> Serial:
	if not os.path.exists(port):
		raise ConnectionRefusedError(port)
	return Serial(
		await saio.open_serial_connection(url=port, baudrate=baudrate, limit=BUFF_SIZE),
		**kwargs
	)
